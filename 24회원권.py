...
# 1. 파일 경로 설정 및 여러 CSV 파일 합치기 (24년 1월 ~ 05월)
...
print("\n2024.01.01 ~ 2024.05.30 시간대별 정기권, 일일권, 비회원 자전거 대여건수, 대여시간(시간), 이동거리(KM), 운동량(칼로리), 탄소량(kg):")
...
for hour in hours:
    ...
    if not hourly_pass_type_rent_count.empty:
        ...
        print(f"\n2024.01.01 ~ 2024.05.30 {hour}시에는 {most_rented_pass_type}이 가장 많이 사용됨 (이동거리: {most_rented_pass_distance:.2f} km)")
    ...
    if not pass_type_data.empty:
        print(f"\n2024.01.01 ~ 2024.05.30 대여시간 {hour}시 정기권, 일일권, 비회원 데이터:")
...
print(f"\n2024.01.01 ~ 2024.05.30 전체 시간 동안 가장 많이 사용된 회원권: {overall_most_rented_pass_type} ({overall_most_rented_pass_count}건)")
...
# 총 데이터 수 계산
total_count = len(df)
print(f"24년 회원권 총 데이터수: {total_count}개")
