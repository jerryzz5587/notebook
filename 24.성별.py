...
# 1. 파일 경로 설정 및 여러 CSV 파일 합치기 (24년 1월 ~ 5월)
...
hours = range(24)
print("\n2024.01.01 ~ 2024.05.30 시간대별 성별 자전거 대여건수, 대여시간(시간), 이동거리(KM) 및 성별 많이 탄 정보:")
...
for hour in hours:
    ...
    if not gender_data.empty:
        print(f"\n2024.01.01 ~ 2024.05.30 대여시간 {hour}시 성별 데이터:")
        print(gender_data)
# 성별별 대여수 계산 및 출력
hours = df['대여시간'].unique()  # 전체 시간대 추출

total_gender_rent_count = pd.Series(dtype='float64')  # 전체 시간대의 성별 대여수 합산을 위한 시리즈 초기화
for hour in hours:
    # 시간대별 성별 대여 건수
    hourly_gender_rent_count = df[df['대여시간'] == hour].groupby('성별').size()
    ...
    if len(hourly_gender_rent_count) > 0:
        ...
        print(f"2024.01.01 ~ 2024.05.30 {hour}시에는 {most_rented_gender}가 많이 탔음 ({most_rented_gender_count}명)")
...
print(f"\n2024년 1분기 시간별 가장 많이 자전거를 대여한 성별은 {overall_most_rented_gender} ({overall_most_rented_gender_count}명)")
...
# 출력
print(f"\n2024.01.01 ~ 2024.05.30 전체 시간 동안 가장 많이 자전거를 대여한 성별은 {overall_most_rented_gender} ({overall_most_rented_gender_count}명)")
print(f"\n2024.01.01 ~ 2024.05.30 전체 기간 동안 총 남자 대여 건수: {total_male_rent_count}명")
print(f"2024.01.01 ~ 2024.05.30 전체 기간 동안 총 여자 대여 건수: {total_female_rent_count}명")
...
# 총 데이터 수 계산
print(f"24년 성별 총 데이터수: {len(df)}개")
