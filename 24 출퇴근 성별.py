...
total_average_distance = total_gender_group_stats['총이동거리(km)'].mean()
total_distance_percentage_above_avg = ((total_gender_group_stats.loc[total_distance_gender, '총이동거리(km)'] - total_average_distance) / total_average_distance) * 100

total_average_time = total_gender_group_stats['총이용시간'].mean()
total_time_percentage_above_avg = ((total_gender_group_stats.loc[total_time_gender, '총이용시간'] - total_average_time) / total_average_time) * 100
...
print(f"이동거리가 가장 많은 성별: {total_distance_gender} (총 이동거리: {total_gender_group_stats.loc[total_distance_gender, '총이동거리(km)']:.2f} km, 대여 1건당 평균 이동거리: {total_gender_group_stats.loc[total_distance_gender, '대여건당평균이동거리(km)']:.2f} km), 다른 성별 평균보다 {total_distance_percentage_above_avg:.2f}% 높음")
print(f"이용시간이 가장 긴 성별: {total_time_gender} (총 이용시간: {total_gender_group_stats.loc[total_time_gender, '총이용시간'] / 60:.2f}시간), 다른 성별 평균보다 {total_time_percentage_above_avg:.2f}% 높음")
